...
const (
	...
	defaultFileContent      = "image/webp"
	defaultFileUpsert       = false
	defaultSortColumn       = "name"
	defaultSortOrder        = "asc"
)

func (f *file) UploadOrUpdate(path string, data io.Reader, update bool, fileType string) FileResponse {
...
func (f *file) Update(path string, data io.Reader, filType string) FileResponse {
	return f.UploadOrUpdate(path, data, true, filType)
}

// Upload uploads a file object to a storage bucket
func (f *file) Upload(path string, data io.Reader, fileType string) FileResponse {
	return f.UploadOrUpdate(path, data, false, fileType)
}
...
func (f *file) Copy(fromPath, toPath string) FileResponse {
	...
	reqURL := fmt.Sprintf("%s/%s/object/copy", f.storage.client.BaseURL, StorageEndpoint)
	req, err := http.NewRequest(http.MethodPost, reqURL, bytes.NewBuffer(_json))
	if err != nil {
		panic(err)
	}
